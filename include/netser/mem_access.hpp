...
namespace netser
{
    ...
    namespace detail
    {
        ...
        template <typename T>
        using is_placed_atomic_memory_access_t
            = std::remove_all_extents_t<decltype(impl::is_placed_atomic_memory_access_func(std::declval<T>()))>;
    ...
    struct placed_atomic_memory_access : public AtomicMemoryAccess
    {
        ...
        {
            static_assert(AlignedPtr::get_access_alignment(byte_offset) % atomic_access::alignment == 0,
                          "Offset does not meet alignment requirements!");
        ...
        namespace impl
        {
            ...
            struct filter_memory_access_list<BufferAlignment, type_list<SourceAccess0, SourceAccesses...>, type_list<ResultAccesses...>>
            {
                using result = typename filter_memory_access_list<
                    BufferAlignment, type_list<SourceAccesses...>,
                    std::conditional_t<((BufferAlignment % SourceAccess0::alignment) == 0), type_list<ResultAccesses..., SourceAccess0>,
                                       type_list<ResultAccesses...>>>::result;
            };
