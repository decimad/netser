...
GTEST_TEST(integer_test, single_read32)
{
    ...
    EXPECT_TRUE(log.size() == 2 && log[0].offset == 0 && log[0].size == 2 && log[1].offset == 2 && log[1].size == 2);
    log.clear();
...
GTEST_TEST(integer_test, single_write48)
{
    ...
    EXPECT_EQ(log[0].size, 4);
    EXPECT_EQ(log[1].offset, 4);
    EXPECT_EQ(log[1].size, 2);
    dest = 0;
    log.clear();
    ...
    ASSERT_EQ(log.size(), 4);
    EXPECT_EQ(log[0].offset, 0);
    EXPECT_EQ(log[0].size, 1);
    EXPECT_EQ(log[1].offset, 1);
    EXPECT_EQ(log[1].size, 2);
    EXPECT_EQ(log[2].offset, 3);
    EXPECT_EQ(log[2].size, 2);
    EXPECT_EQ(log[3].offset, 5);
    EXPECT_EQ(log[3].size, 1);
    dest = 0;
    log.clear();
    ...
    ASSERT_EQ(log.size(), 2);
    EXPECT_EQ(log[0].offset, 0);
    EXPECT_EQ(log[0].size, 2);
    EXPECT_EQ(log[1].offset, 2);
    EXPECT_EQ(log[1].size, 4);
    dest = 0;
    log.clear();
    ...
    EXPECT_EQ(log[1].offset, 1);
    EXPECT_EQ(log[1].size, 4);
    EXPECT_EQ(log[2].offset, 5);
    EXPECT_EQ(log[2].size, 1);
    dest = 0;
    log.clear();
}
